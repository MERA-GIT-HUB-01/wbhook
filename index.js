...
const mytoken = process.env.MYTOKEN; //Ashish_token
let phon_no_id;
let from;
...
const webhook = new WebhookClient({
    channel: {
        url: 'https://oda-7232c4c71e634aee9889ca2658e4ce67-da3.data.digitalassistant.oci.oraclecloud.com/connectors/v2/listeners/webhook/channels/3472bba8-a722-409c-b301-e09021c84005',
        secret: 'r1Mx9Sd0oCiX1dkfodBBW8hWDnmIJ7nB'
    }
});

webhook
    .on(WebhookEvent.ERROR, err => console.log('HRMS webhook Error:', err.message))
    .on(WebhookEvent.MESSAGE_SENT, message => console.log('HRMS Message to chatbot:', message));
app.post('/bot/message', webhook.receiver()); // receive bot messages


app.listen(process.env.PORT, () => {
    console.log("Hi Centralite. your Webhook is listening");
});
...
app.post("/webhook", (req, res) => { //i want some 
    ...
    if (body_param.object) {
        console.log("Ashish i am inside body");
        if (body_param.entry &&
            ...
            console.log("Ashish your Message before sending to ODA is ------>" + message);
...
app.get("/", (req, res) => {
    res.status(200).send("Hello Ashish this is webhook setup");
});
